...
func placeBlock(terrain, position: Vector3i, blockId: int, rotation=0, variant=0, color=0, storageData=null):
	var item = blockUtils.list_id2obj[blockUtils.getVariantBlockId(blockId, rotation, variant, color)]
...
func destroyBlock(terrain, position: Vector3i):
	var item = blockUtils.list_id2obj[terrain.voxel_tool.get_voxel(position)]

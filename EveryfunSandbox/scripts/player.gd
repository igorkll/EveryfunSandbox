	...
	if not controlLock:
		...
		if Input.is_action_just_pressed("place"):
			...
			if result and terrainUtils.isCellFree(game.terrain, result[1].previous_position):
				terrainInteractions.placeBlock(game.terrain, result[1].previous_position, game.blockIDs["lamp"], game.getBlockDefaultRotation($camera.global_transform.basis.z))

...
func setRenderDistance(index):
	...
	lod_count = distanceSettingsPreset.lodCount
...
func _ready():
	var guiContainer = get_node("/root/main/gui")
	guiContainer.add_child(preload("res://gui/gameGui.tscn").instantiate())
	guiContainer.add_child(preload("res://gui/gameMenu.tscn").instantiate())
